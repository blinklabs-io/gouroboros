...
// ErrLocalPeerSharingDisabled is retained for callers that classified the
// former strict rejection behavior. The server now answers an unexpected
// request with an empty SharePeers response so an optional protocol cannot
// tear down the shared bearer.
...
// the handshake. The server answers an incoming ShareRequest with an empty
// SharePeers response when this flag is set.
