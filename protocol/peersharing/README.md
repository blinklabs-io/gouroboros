...
| NoPeerSharing | active | no — would be a self-protocol violation; client should not be calling `GetPeers` | yes — an empty `SharePeers` response |
...
but the defensive response keeps this optional mini-protocol from closing the
shared connection if it does.
