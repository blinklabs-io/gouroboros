...
func TestValidateSlotLeader(t *testing.T) {
	...
	if err != nil {
		t.Errorf("expected no error when no genesis keys configured, got: %v", err)
	}
}
...
func TestSlotLeader(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if index != tc.expectedIndex {
			t.Errorf("SlotLeader(%d) index = %d, want %d", tc.slot, index, tc.expectedIndex)
		}
		if keyHash == nil {
			t.Errorf("SlotLeader(%d) returned nil key hash", tc.slot)
		} else if keyHash[0] != byte(tc.expectedIndex) {
			t.Errorf("SlotLeader(%d) returned wrong key hash", tc.slot)
		}
	}
	...
	if index != -1 {
		t.Errorf("SlotLeader with no keys should return -1, got %d", index)
	}
	if keyHash != nil {
		t.Errorf("SlotLeader with no keys should return nil hash")
	}
}
