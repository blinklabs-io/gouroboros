...
const conwayGenesisConfig = `
{
  ...
  "initialDReps": {
    "keyHash-4959a3d07075fe7d86bda0ce93e334950c4243729a77968cd4c37abf": {
      "deposit": 500000000,
      "expiry": 1000
    }
  },
...
func TestGenesisFromJson(t *testing.T) {
	...
	assert.Equal(t, tmpGenesisDelegK, expectedGenesisDelegK)
	assert.Equal(t, tmpGenesisDelegV, expectedGenesisDelegV)
	tmpGenesis.Delegs = nil
	expectedGenesis.Delegs = nil
	// Compare initial DReps separately
	// This expects that there is only a single initial drep in the test and will likely break with more
	var tmpGenesisInitialDRepK common.Credential
	var tmpGenesisInitialDRepV conway.ConwayGenesisDRepState
	for k, v := range tmpGenesis.InitialDReps {
		tmpGenesisInitialDRepK = *k
		tmpGenesisInitialDRepV = v
	}
	var expectedGenesisInitialDRepK common.Credential
	var expectedGenesisInitialDRepV conway.ConwayGenesisDRepState
	for k, v := range expectedGenesis.InitialDReps {
		expectedGenesisInitialDRepK = *k
		expectedGenesisInitialDRepV = v
	}
	assert.Equal(t, tmpGenesisInitialDRepK, expectedGenesisInitialDRepK)
	assert.Equal(t, tmpGenesisInitialDRepV, expectedGenesisInitialDRepV)
	tmpGenesis.InitialDReps = nil
	expectedGenesis.InitialDReps = nil
	// Compare remaining structure
	assert.Equal(t, tmpGenesis, expectedGenesis)
}
