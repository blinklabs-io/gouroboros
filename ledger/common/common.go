...
func ExtractTransactionOffsets(cborData []byte) (*BlockTransactionOffsets, error) {
	...
	if len(blockArray) < 3 {
		...
		return &BlockTransactionOffsets{}, nil
	}
...
// extractDatumOffsets extracts datum hash -> offset mappings from a datum map using streaming decoding.
// The datum map structure is: { datum_hash: datum_cbor, ... }
func extractDatumOffsets(datumMapData []byte, baseOffset uint32, result map[Blake2b256]ByteRange) {
	if len(datumMapData) < 2 {
		return
	}

	// Get map info from header
	count, headerSize, indefinite := cborMapInfo(datumMapData)
	if count < 0 && !indefinite {
		return // Invalid map header
	}

	// Use streaming decoder starting after the map header
	dec, err := cbor.NewStreamDecoder(datumMapData[headerSize:])
	if err != nil {
		return
	}
	...
	for i := 0; indefinite || i < count; i++ {
		...
		if indefinite {
			pos := dec.Position()
			if int(headerSize)+pos >= len(datumMapData) {
				break
			}
			if datumMapData[int(headerSize)+pos] == 0xff {
				break
			}
		}

		// Try to decode key as bytes (datum hash)
		var keyBytes []byte
		if _, _, err := dec.Decode(&keyBytes); err != nil {
			// Key might be a different type, skip the pair
			return
		}

		// Get value position and raw bytes
		valueOffset, valueBytes, err := dec.DecodeRaw(new(cbor.RawMessage))
		if err != nil {
			return
		}

		// Determine hash: use key if it's a valid hash, otherwise compute from value
		var hash Blake2b256
		if len(keyBytes) == Blake2b256Size {
			copy(hash[:], keyBytes)
		} else {
			// Compute hash from datum bytes
			computed := blake2b.Sum256(valueBytes)
			copy(hash[:], computed[:])
		}

		result[hash] = ByteRange{
			Offset: baseOffset + headerSize + uint32(valueOffset), // #nosec G115 -- Cardano block segments are <<4GiB
			Length: uint32(len(valueBytes)),                       // #nosec G115 -- Cardano block segments are <<4GiB
		}
	}
}

// extractRedeemerOffsets extracts redeemer key -> offset mappings.
// Redeemers can be either a map (Alonzo-Babbage) or array (Conway+).
func extractRedeemerOffsets(redeemerData []byte, baseOffset uint32, result map[RedeemerKey]ByteRange) {
	if len(redeemerData) < 2 {
		return
	}

	// Check if it's a map or array by looking at the first byte
	firstByte := redeemerData[0]

	if firstByte >= 0x80 && firstByte <= 0x9f || firstByte == 0x9f {
		// It's an array (Conway+ format): [[purpose, index, data, exunits], ...]
		extractRedeemerArrayOffsets(redeemerData, baseOffset, result)
	} else if firstByte >= 0xa0 && firstByte <= 0xbf || firstByte == 0xbf {
		// It's a map (Alonzo-Babbage format): {[purpose, index]: [data, exunits], ...}
...
// Map format (Alonzo-Babbage): {[purpose, index]: [data, exunits], ...}
...
// extractRedeemerArrayOffsets handles the array format for redeemers (Conway+) using streaming decoding.
...
func cborArrayInfo(data []byte) (int, uint32, bool) {
	...
	case additional == 26 && len(data) >= 5:
		return int(uint32(data[1])<<24 | uint32(data[2])<<16 | uint32(data[3])<<8 | uint32(data[4])), 5, false
	case additional == 31:
