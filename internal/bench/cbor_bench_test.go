	...
	testTxs = []testTxData{
		{
			...
			cbor:   mustDecodeHex(byronTxCborHex),
		},
		{
			name:   "Shelley",
			txType: ledger.TxTypeShelley,
			cbor:   mustDecodeHex(shelleyTxCborHex),
		},
		{
			name:   "Allegra",
			txType: ledger.TxTypeAllegra,
			cbor:   mustDecodeHex(allegraTxCborHex),
		},
		{
			name:   "Mary",
			txType: ledger.TxTypeMary,
			cbor:   mustDecodeHex(maryTxCborHex),
		},
		{
			name:   "Alonzo",
			txType: ledger.TxTypeAlonzo,
			cbor:   mustDecodeHex(alonzoTxCborHex),
		},
		{
			name:   "Babbage",
			txType: ledger.TxTypeBabbage,
			cbor:   mustDecodeHex(babbageTxCborHex),
		},
		{
			name:   "Conway",
			txType: ledger.TxTypeConway,
			cbor:   mustDecodeHex(conwayTxCborHex),
		},
	}
}
...
// BenchmarkCBOREncodeBlock benchmarks block.Cbor() retrieval by era.
// Note: For DecodeStoreCbor-backed types (decoded via NewBlockFromCbor), Cbor()
// returns the cached original bytes rather than re-encoding. This benchmark
// measures the Cbor() method as called in production code paths.
...
// BenchmarkCBOREncodeTx benchmarks transaction.Cbor() retrieval by era.
// Note: For DecodeStoreCbor-backed types (decoded via NewTransactionFromCbor),
// Cbor() returns the cached original bytes rather than re-encoding.
...
// This measures the complete serialization overhead.
