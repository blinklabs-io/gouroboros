...
func TestMaryTransaction_Utxorpc(t *testing.T) {
	...
	if got.Fee.GetInt() != 25 {
		t.Errorf("Transaction fee mismatch: got %d, want 25", got.Fee.GetInt())
	}
	if len(got.Inputs) != 1 {
		t.Errorf("Expected 1 input, got %d", len(got.Inputs))
	}
	...
	if bigInt := coin.GetBigUInt(); bigInt != nil {
		coinValue := new(big.Int).SetBytes(bigInt).Uint64()
		if coinValue != uint64(8000) {
			t.Errorf(
				"Output coin mismatch: got %d, want 8000",
				coinValue,
			)
		}
	} else if coin.GetInt() != 8000 {
		t.Errorf("Output coin mismatch: got %d, want 8000", coin.GetInt())
	}
	if len(got.Hash) == 0 {
		t.Error("Expected non-empty transaction hash")
	}
...
func TestMaryUtxorpc_WithMinPoolCost(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if bigUInt := result.MinPoolCost.GetBigUInt(); bigUInt != nil {
				got := new(big.Int).SetBytes(bigUInt).Uint64()
				if got != tc.expectedMinPoolCost {
					t.Errorf(
						"MinPoolCost mismatch: got %d, want %d",
						got,
			...
			} else if intVal := result.MinPoolCost.GetInt(); uint64(intVal) != tc.expectedMinPoolCost {
				t.Errorf(
					"MinPoolCost mismatch: got %d, want %d",
					intVal,
					tc.expectedMinPoolCost,
...
// TestMaryUtxorpc_MandatoryRatFieldNilEmbeddedRatRejectedNotPanic is the
// regression test for blinklabs-io/gouroboros#2296: A0, Rho, and Tau are
// mandatory *cbor.Rat fields whose nil check only covered the outer
// pointer, not whether a non-nil *cbor.Rat's embedded *big.Rat was itself
// nil. That shape reached Num()/Denom() calls on a nil receiver and
// panicked instead of being rejected with the same error a nil outer
// pointer already gets. Ported from conway's regression test for the same
// finding.
func TestMaryUtxorpc_MandatoryRatFieldNilEmbeddedRatRejectedNotPanic(
	t *testing.T,
) {
	validBase := func() mary.MaryProtocolParameters {
	...
	tests := []struct {
		name   string
		mutate func(*mary.MaryProtocolParameters)
	}{
		{"A0", func(p *mary.MaryProtocolParameters) {
			p.A0 = &cbor.Rat{}
		}},
		{"Rho", func(p *mary.MaryProtocolParameters) {
			p.Rho = &cbor.Rat{}
		}},
		{"Tau", func(p *mary.MaryProtocolParameters) {
			p.Tau = &cbor.Rat{}
		}},
	}
...
// TestMaryUtxorpc_ValueBeyondInt64RangeRejected is the regression test for
// blinklabs-io/gouroboros#2296: the range check compared r.Num().Int64()
// against math.MinInt32/MaxInt32, but big.Int.Int64() is undefined
// (silently wraps, per math/big's own documentation) for a value that does
// not fit in int64 at all. A numerator like 2^64+1 could pass that
...
func TestMaryUtxorpc_ValueBeyondInt64RangeRejected(t *testing.T) {
